...
##  <a name="notebookBegin"> Lumping and Splitting </a>

```{r hide=TRUE}
require('shiny')
#inclRmd('Overview.Rmd')
## Yes, you can have 2 inclRmd in one file.
## Either in the same chunk or separate.
```
```{r hide=TRUE}
require('shiny')
...
```{r}
shiny::includeHTML('Rmd-text-snippets/parametrize.html')
...
#### <a name='Bayes_joint_prior'> Approach: Bayes mixture of "Lump" and "Split"</a>
...
Another Bayesian approach is to set a Bayesian prior on the joint distribution of the two conditional probabilities Pr(R|D) and Pr(R|L). This allows data on the two probabilities to be shared between them, without the extreme "Dr. Lump".
```{r}
QandAha()
```
