...
We also convert the observed proportions to logits. We estimate the variances using the delta method (see `r linkout('www/the-delta-method.pdf')`) applied to the Poisson distribution, whereby the variance of the logarithm of a count is roughly the reciprocal of the count.
...
###	Strong prior belief that $X$ is *un*important
...
* `r linkinLink('a_crossvalidationPlot', 'Cross-validation plot')`
  (an advantage of linkinLink--if it worked:  capturing the current location for return)
* [Cross-validation plot](#a_crossvalidationPlot)
* [Bivariate logit plot](#a_viewPlot)
...
###	Strong prior belief that $X$ is important

What if D and L represent two alleles of a gene believed to affect this drug's pharmacodynamics? Or perhaps, D = dark hair, L = light hair, but hair color is strongly tied to ethnicity in this sample … which is strongly tied to a key genetic variant.
...
* TODO: links to previous figures and results.
* TODO: a way to return to here.
...
**One key point**: none of these methods utilizes prior belief about the plausibility of the features. To utilize prior belief, 

### Two alleles of one gene out of a hundred thousand tested; nothing known

The Benjamini-Hochberg method is often used in this situation. 
