...
What if D and L represent two alleles of a gene believed to affect this drug's pharmacodynamics? "Dr. Who" can express this by choosing a prior odds that favors Dr. Split over Dr. Lump. Experiment by choosing such a value here, 
```{r WhoPriorOdds}
numericInput(inputId = 'WhoPriorOdds', label = 'Dr.Who\'s Prior Probability ,Pr(Split is correct)', value = 0, min = 0, max=1, step = 0.1)
```

and reviewing previous results at these locations:
...
As before, suppose that we have features for a hundred genes which were *previously* identified as known to affect the pharmacodynamics of the drug being tested. Our feature $X$ is select to be the best of the bunch.
...
TextQuestion(' How would you modify the "classical" approach that chooses Lump or Split based on a hypothesis test?')
```
-> Revisit [Classica bifurcation](#ClassicalTest) 
