...
DLdataOriginal = matrix(c(3,5,2,90),nrow=2)
dimnames(DLdataOriginal) = list(c("D","L"),c("R","N"))
...
firstCellIds = paste0('m', cellNames, 'idPanelDTC1')
names(firstCellIds) = cellNames
updateDLdata = observeEvent(
  priority = 2,
  label='updateDLdata',
  eventExpr = #c(input$mRD, input$mRL, input$mND, input$mNL)
    c(input[[firstCellIds[1]]], 
      input[[firstCellIds[2]]],
      input[[firstCellIds[3]]], 
      input[[firstCellIds[4]]]),
  handlerExpr = {
    try(silent = TRUE, {
      isolate({
        cat('input$mRD=', input[[firstCellIds[1]]], 
            '  rValues$resetting=', rValues$resetting, '\n')
        ...
        rValues$DLdata[1,1] =  input[[firstCellIds[1]]]
        rValues$DLdata[2,1] =  input[[firstCellIds[2]]]
        rValues$DLdata[1,2] =  input[[firstCellIds[3]]]
        rValues$DLdata[2,2] =  input[[firstCellIds[4]]]
        if(rValues$resetting ==  FALSE) { 
...
# updateDLdataMyChoice = observeEvent(label='updateDLdataMyChoice',
#     eventExpr = c(input$mRD, input$mND, input$mRL, input$mNL), 
#       handlerExpr = {
#          print('changing DLdataMyChoice also')
#           rValues$DLdataMyChoice[1,1] = input$mRD
#           rValues$DLdataMyChoice[1,2] = input$mND
#           rValues$DLdataMyChoice[2,1] = input$mRL
#           rValues$DLdataMyChoice[2,2] = input$mNL
#       }
# )
```
...
## 2sharps The Lump versus Split Dilemma
