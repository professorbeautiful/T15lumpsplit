...
```{r Overview}
output[['overviewId']] <- renderUI({
  list(
    checkboxInput('cbOverviewId', 
                  strong(em("show/hide the Overview")),
                  value=TRUE),
    conditionalPanel(condition = 'input.cbOverviewId',
                     tagList(inclRmd("Overview.Rmd")) )
  )
})
uiOutput('overviewId')
```
...
output[['navTOCid']] <- renderUI({
  list(
    checkboxInput(cbInputId, 
                  strong(em("show/hide outline of topics")),
                  value=FALSE),
