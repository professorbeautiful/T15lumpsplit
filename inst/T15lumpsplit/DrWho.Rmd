...
$\color{green}{Dr. Who}$ doesn't know **who** to believe.   
but sticks with the idea  
that one and only one is correct.  
Maybe one’s correct ($\phi  = \color{blue}{Split}$ ),  
maybe the other one is ( $\phi  = \color{red}{Lump}$). 
...
```{r fig.width=8, fig.height=6, fig.margin=TRUE}
...
output$whoMixturePlot = 
  renderPlot(expr =  {
    par(mar = c(3, 3, 1, 1))   
            #  wait, this works??  warning no longer! and mixture plot is ok.

    #req(input$whoMixturePlot_width > 200, cancelOutput = FALSE)
    #  this works to suppress the warning msg, but also blocks the mixture plot entirely
   # withCallingHandlers(
   # 
   #  error = function(e) {
   ...
   #   expr = {
    ...
    WhoPriorProb = rValues$WhoPriorProb
    ...
    prior.w = WhoPriorProb * prior.s + (1-WhoPriorProb) * prior.l
    plot(pSeq, prior.s, col='blue', type='l', lwd=lwd.s,
         xlab='probability', ylab='density',
         ylim=c(0,15),
         main='Priors') 
    lines(pSeq, prior.l,  col='red', lwd=lwd.l)
    lines(pSeq, prior.w,  col='darkgreen', lwd=lwd.w)
...
#currentTableOfEstimates = tableOfEstimates()
    ...
    pProbSplit = posteriorProb(priorOdds = WhoPriorProb, theData=thisData)
    ...
    lines.default(pSeq, posterior.w,  col='darkgreen', lwd=2)
    ...
    text(offset+0.65, 5, labels = "L",
         col='red', cex=1.5, pos=4)
    onTop = par('usr')[4] * 1.05  ## ['y2']
    #cat('onTop: ', onTop, '\n')
    ######  Mark the positions of the means.
    text(0, onTop, col='black', xpd=NA, 
         labels=expression(Means %=>% phantom('.')), cex=1, pos = 2, font=4)
    letterVerticalPosition = -0.5 * par('usr')[4]
    letterVerticalPosition = onTop
    addCircledLetter(
      pointLocation = c(posterior.mean.s, 0),
      labelLocation = c(posterior.mean.s, letterVerticalPosition),
      pch='S', col='blue', cex=1.)
    addCircledLetter(
      pointLocation = c(posterior.mean.l, 0),
      labelLocation = c(posterior.mean.l, letterVerticalPosition),
      pch='L', col='red', cex=1.)
    addCircledLetter(
      pointLocation = c(posterior.mean.w, 0),
      labelLocation = c(posterior.mean.w, letterVerticalPosition),
      pch='W', col='darkgreen', cex=1.)
...
```{r}
conditionalPanelWithCheckboxPDF(
  labelString='(B)  Apply Bayes “theorem”:  Calculate the (posterior) distribution of the unknown quantities', 
