...
K(\mylarge{\bf{n}})
$$  
that depends only onx the observed data $\mylarge{\bf{n}}$  
...
####	If **the sample size is fixed <br> by the study's design** at 100,  
...
####	If the **two bottom margins are fixed  <br> by the study's design**,  
...
####	If **both margin counts are fixed**  <br> by the study's design,  
then the model is *hypergeometric*,  
and the natural parametrization is  
$({\mylarge{p}_D},{\mylarge{p}_R},~\mylarge{\phi} )$  
where $\phi$ is the odds ratio.  (Details are omitted.)  
But that sampling model is impossible to implement  
in a clinical study design,  
even though when one uses the Fisher "exact" test  
one is pretending that it is the sampling model.  

**SURPRISE!** 
they all have the same likelihood function!  
