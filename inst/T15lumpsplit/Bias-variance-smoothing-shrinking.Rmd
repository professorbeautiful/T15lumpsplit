...
includeScript('www/onbeforeunload.js')
```
...
What is bias? What is variance? How are they related to lumping and shrinking?
...
### Approaches to estimating Pr(R|D)
...
observeEvent(list(input$regenerateFeatures, rValues$DLdata), {
    rValues$DLdataDFwithFeatures = 
       makeDLdataWithFeatures(rValues$DLdata)
})
```
...
For our original D/L feature, with P value `r allChisqPs_original[1]` (compare with `r reactive({rValues$P_Q_for_X_DL_original[1]})`), the Q-value is `r reactive({rValues$P_Q_for_X_DL_original[2]})`. 
