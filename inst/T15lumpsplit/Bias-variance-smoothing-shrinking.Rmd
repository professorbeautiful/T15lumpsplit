...
output$estimateTable = 
  ...
  result = xtable::xtable(data.frame(
      `Raw (Lump)`=LumpRawMean,
      `Raw (Split)`=SplitRawMean,
      `Bayes Mean (Lump)`=posterior.mean.l,
      `Bayes Mean (Split)`=posterior.mean.s ,
      `Bayes Mean (Who)`=posterior.mean.w,  
      `Bayes Pr(Lump)`=pProbLump,
      `Bayes Pr(Split)`=pProbSplit
