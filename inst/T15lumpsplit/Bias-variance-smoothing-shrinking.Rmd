...
makeSureTauTrueIsGood = function(tauTrue) {
  tauTrue = as.numeric(tauTrue)
  if(
    is.na(tauTrue)
    | is.nan(tauTrue)
    | ! (length(tauTrue)==1) )
...
#              )
#              allChisqPs <<- rValues$allChisqPs = Pvalues$allChisqPs
...
```{r oddsratiofunctions}
  fun.oddsratio = function(col, sprinkle=1) {
  ...
  fun.logitRatioVariance = function(col, sprinkle=0.1) {
    theTable = table(col, outcome)
    theTable = sprinkle + theTable
    sum(1/theTable)
  }
...
fun.oddsratioplot = function() {
  ...
  oddsratios = sapply(thisBigData[-(1)], fun.oddsratio)
  ...
  points(logitRatios, col='red', type='h',
       logitRatioSDs * par('usr')[4]/
        max(logitRatioSDs) *0.3)
  points(logitRatioSDs[1], col='green', type='h',
       logitRatioSDs[1] * par('usr')[4]/
        max(logitRatioSDs) *0.3,
       lwd=2)
  text(logitRatios[1], 0.2, cex=2,
...
fun.empBayeslogitRatios = function(){
  ...
  weights = 1 - tau / (rValues$logitRatioVariances + tau)
