...
observeEvent(
  list(input$regenerateFeatures, 
       rValues$qStar),  
  ### Do NOT recompute extra features when rValues$DLdata changes.
  {
...
output$qvaluePlot = renderPlot({
  ...
  plot(qout.mod$pvalues, qout.mod$qvalues, 
       xlim=c(0, 1.1 * max(rValues$P_Q_for_X_DL_modified,
                     sort(qout.mod$pvalues)[1:8][8]) ), 
       ylim=c(0, 1.1 * max(rValues$P_Q_for_X_DL_modified,
  ...
  text(0.0002, 0.03, 'Bonferroni', cex=2, srt=75, col='darkred')
  abline(a=0, b=1, col='blue')
  text(0.0025, 0.004, 'diagonal', cex=2, srt=4, col='blue')
  #plot(qvalue::qvalue(rValues$allChisqPs) )
})
plotOutput('qvaluePlot')
```

From this we see that the Q-value for our original D/L feature is `r reactive({rValues$P_Q_for_X_DL[2]})`. 
