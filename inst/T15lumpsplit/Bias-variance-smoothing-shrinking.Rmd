...
fun.empBayeslogitRatios = function(){
  ...
  mu = mean(logitRatios)
  tau = 0   #(var(logitRatios) - mean(logitRatioVariances))
  for( irep in 1:13) { # we ain't buildin' no cathedral here.
    ...
    if(max(abs(mu.last-mu), abs(tau.last-tau)) < 1e-4)
      break
  }
  
  rValues$tauEstimateEB = tau
  cat('mu-hat: ', mu, '  tau-hat: ', tau, '\n')
  weights = 1 - tau / (logitRatioVariances + tau)
  ...
  lines(c(logitRatios[iobs], rValues$posteriorMeans[iobs]),
        0:1)
  posteriorVariance_DL =  weights[1] * logitRatioVariances[1]
...
output[[thisTauTrueID_text]] = renderText(
  signif(digits=4, input[[thisTauTrueID]]))
```

The estimated variance of the logits is `r  textOutput('tauEstEB', inline=TRUE)`. (The value that generated the data is `r  textOutput(thisTauTrueID_text, inline=TRUE)` ) .
