---
title: "The group  vs &nbsp;&nbsp; the individual.<br>Lumping &nbsp;&nbsp;&nbsp;vs &nbsp;&nbsp;splitting.  <br>Bias &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;vs &nbsp;&nbsp;variance."
resource_files:
- www/zoomAdvice.Rmd
...
- 3 4 3 90 table fixed.png
- table fixed margins.png
runtime: shiny
output:
  html_document:
    number_sections: yes
...
```{r invisible packages, context="server", message=FALSE, warning=FALSE}
...
myErrorFunction <<- function(){
  dump.frames()
  showModal(modalDialog(title='ERR', 
                        last.dump))
}
options(error = myErrorFunction) 
...
#   To easily repeatedlyrevisit a location on the page, I use a KeyboardMaestro macro called "search web page for a phrase"
...
require(knitr);require(rmarkdown);require(markdown);
require(reactlog)
...
splitColor = 'blue'
whoColor = 'green'
colorsForDoctors = c(lump=lumpColor, split=splitColor, who=whoColor)
...
```{r servercode}
DLdata = list()
rValues = reactiveValues(tau = 0.5,  phi = 0.5, mu = 0.5,
                         title_1='title 1',
                         #DLdata=list(),
                         #DLdataMyChoice=list(),
                         #DLdataLastUsed = DLdataOriginal,
                         isResetting = FALSE,  #not used
                         isLoopingSync = FALSE, #not used
                         WhoPriorProb = 1/2,
                         WhoPriorOdds = 1, #,
...
```{r sessioninitialized, eval=TRUE}
...
options(shiny.reactlog=TRUE)   ### TEMPORARY
...
#  incidentally, this works in shinyDebuggingPanel:   click('whoID')
...
```{r}
go_to_section =  function(sectionName='whoMixturePlot', 
                          labelString=paste0('▸ ', jumpList_DTC[[sectionName]])){
  anchorName = paste0('a_', sectionName)
  thisNumber = which(names(jumpList_DTC) == sectionName)
  IdThisLinkin = paste0('linkinLink', thisNumber)
  onclickAction = paste0(IdThisLinkin, labelString)
  # This works, even when pasted into debug panel!
  a(id=IdThisLinkin,
...
# if(file.exists('gitbranch.txt'))
    # gitbranchText = paste('Branch: ', readLines('gitbranch.txt'))
```
*Package:T15lumpsplit  `r packageVersion('T15lumpsplit')`  `r packageDate('T15lumpsplit')` `r gitbranchText`  <br>  Author: Roger Day <br> An interactive "shiny" app. Visit [statisticsbeautiful](https://www.youtube.com/@statisticsbeautiful) for videos.*
...
conditionalPanelWithCheckbox( #ADVICE
...
# "Lumping" versus "splitting" <br>in science and medicine. <!-- #title -->

You are a unique individual.  
**Nobody** else is like you.  
You are part of humanity.  
**Everybody** else is like you.  
 You are split off from humanity,  
 but also lumped with all humanity.

So here's our dilemma.  
**Lump you, or split you**?   
Or can we do something **better than both**?
...
Here is a deeper dive into lumping and splitting.



```{r}

conditionalPanelWithCheckbox(  #DEEPER DIVE
  labelString = 'deeper dive into lumping and splitting', 
html=(shiny::includeMarkdown(path = 'LumpSplitSidebar.Rmd')),
...
We will explore a simple lumping and splitting conundrum  
as a jumping off point for introducing a collection  
of important statistical ideas that connect to each other.
...
# The Lump vs Split challenge in a medical study <!-- #title -->


**The Problem**:  A new treatment is given to some patients.  


A  patient who gets a good response has $Outcome=R$;  
if it's a bad response, that's  $Outcome=N$,  $N$ for non-responding.


<img src="3 4 3 90 table fixed.png" width="50%">

Of `r sum(DLdataOriginal)` patients, only `r sum(DLdataOriginal["R", ])` respond.    
That is discouraging. 

But there is a tiny subgroup of just `r sum(DLdataOriginal[ , "D"])` in which `r sum(DLdataOriginal["R","D"])` patients respond,  
yielding a response rate of `r round(100*sum(DLdataOriginal["R","D"])/sum(DLdataOriginal[ , "D"]) )`%.  
For now we call them "$D$ patients",  
and the others "$L$ patients".  
Which group a patient belongs to is a *"feature" ("predictor", "covariate")*  

This feature might be a valuable biomarker.   
Or it might be nonsense.  
Possibilities for its actual identity  will be explored later.  

**The Challenge**:  
Decide whether to treat the "$D$ patients"  
-- or more modestly, to decide whether to put resources  
into studying the treatment further for "$D$ patients".  
To guide the decision, we need to assess what we know about the  
**conditional probability**,  
$Pr(R|D)$,  
the probability that the patient responds ($R$) to treatment  
given that the patient is a $D$ type of patient.  


```{r}
conditionalPanelWithCheckbox('Other ways that people write conditional probability' , filename = 'conditionalProbabilityModel.Rmd')
``` 


## <a class='twosharps' name='section-Lump-or-Split'> Changing the data: estimates & intervals</a> <!-- #title -->

In the following plot, we see the estimate and the confidence interval  
for `Pr(R | D)`, depending on whether  
the *"Lump"* or *"Split"* approach is taken.  
...
#dataTableComponent(showhide='show', analysisName='')
# nice try but messes up everything else.
```

(Throughout this Lump/Split "app",  
you can change the numbers in the table and observe the results
changing in all the sections.
You can also press "reset" .)
...
## Estimating Pr(R | D), several APPROACHES <!-- #title -->


* [APPROACH: Dr.Lump and Dr.Split have opinions about $Pr(R ~|~ D)$ !](#LumpOrSplit) 
* [APPROACH: Who's right? Gotta choose! Classical significance test](#ClassicalTest)
* [APPROACH: Bayes mixture:  "Dr. Who"](#Bayes_mixture)
* [APPROACH: Bayes joint prior, logit scale](#Bayes_joint_prior).  
* [APPROACH: Cross-validation to optimize](#Cross_validation)
* Approaches used in the famous ECMO data set. (ToDo)


### <a class='threesharps' name='section-LumpOrSplit'>APPROACH: Dr.Lump and Dr.Split reach different conclusions </a> <!-- #title -->
...
output$plotLumpSplitPoints = renderPlot(
  height = 300, 
  {
    #  NOTE!   To get this plotLumpSplitPoints, 
    #we use calculatePlightPdarkPosterior with options to hide W and contours.
    analysisName = 'plotLumpSplitPoints'
    source(analysisReactiveSetup_DTC, local=TRUE)
    ...
    if(inherits(bivariateNormResults, "try-error"))
    return(NULL)
...
**Dr. Split** speaks:  
“I believe that $X_{DL}$  is an important *biomarker*  
-- so important that the $D$ people and the $L$ people  
are fundamentally different.   
Since the question is what to do with $D$ people,  
I will only look at their data.”  

**Dr. Lump**   disagrees!   
“No no, I am sure that this so-called *'biomarker'* $X_{DL}$  
is *irrelevant!*  
I will ignore it, and look at the data  
for *all* the patients.  
My estimate will be more accurate.” 


*Dr. Split* will *"split"* the sample of patients into the two groups,  
estimating the response rate `Pr(R | D)` by utilizing the D patients only,  
is 
`r reactive(round(100*rValues$estimates_Lump_Split$split))`*%*. 

*Dr. Lump* will  *"lump"* together all the patients,  
estimating that the response rate `Pr(R | D)`  
for $D$ patients is the same as `Pr(R)`,  
utilizing the whole set of patients,  
which is `r reactive(round(100*rValues$estimates_Lump_Split$lump))`*%*. 
  


Lumping gives an answer with  
**low variance**  
($N$=100 initially; the sample size is fairly high)  
but **high bias** --  
because the answer reflects far more $L$ patients than $D$ patients.  
So we get a precise answer to what might be the wrong question.  
The confidence interval follows the diagonal of the plot  
on the left above, where $Pr(R~|~D)=Pr(R~|~L)$.

Splitting gives an answer with  
**high variance**  ($N$=6 initially)  
but **low bias**  
because it is including only patients who ARE in the $D$ group:  
it is directly asking the right question,  
but with little precision.  
The very wide horizontal confidence interval  
reflects this *high variance*.



#### <a class='foursharps' name='section-Variance-Bias'> Variance and bias refresher</a> <!-- #title -->

#### <a class='foursharps' name='section-bias-variance-mse'>Bias, variance, and mean squared error</a> <!-- #title -->


For a refresher on the concepts of  
**bias, variance, and mean squared error** in estimation,  
open this panel.
<!--
https://www.geeksforgeeks.org/html/how-to-embed-pdf-file-using-html/
<div>
bias-variance-meansquarederror-MSE.pdf,
         <embed class="pdf"   src="bias-variance-meansquarederror-MSE.pdf"
           width="800" height="500">
    </div>
    OK but not better.  No "Microsoft" label now.  Why changed?
-->
...
#### <a class='foursharps' name='section-bias-variance-axis'> The many faces of the bias-variance axis</a> <!-- #title -->

The bias-variance axis has many disguises:
...
### <a class='threesharps' name='section-ClassicalTest'> APPROACH: Who's right? Dr.Who's gotta choose! Classical significance test</a> <!-- #title -->

We COULD do an old fashioned significance test:  
  ***"Is $X_{DL}$ independent of the outcome Y?"***
A time-honored but fading technique   
to guide which of two analyses "Lump" or "Split", to present.  
  
Does the response rate of D patients  differ from L patients?  
Then, depending on the significance test result,  
Dr.Lump's answer or Dr. Split's answer is selected.  

In a 2x2 table like this one,
one test frequently performed to test whether a feature like  $X_{DL}$  is
independent of an outcome like $R$ or $N$
is the *Fisher exact test*.  
On the original data, the P value is 
**`r textOutput('fisherResultOriginal', inline=TRUE)`.** 

If the P-value is "significant"  
(which "by tradition" means "less than 5%"),  
the "null hypothesis" (that "Dr. Lump" is correct) is rejected.    
With the original data,  
the traditional consequence is that we use Dr. Split's estimate:   

50%...  and WE COMPLETELY IGNORE THE DATA FOR THE "L" PATIENTS!  



```{r Test_to_choose_who_is_right}

Test_to_choose_who_is_right = function(thisData, chosentest=fisher.test) {
    Pvalue = chosentest(thisData)$p.value
    ...
    confInterval_Lump = binom.test(x = sum(thisData['R', ]),
                                   n = sum(thisData[ , ]) )$conf.int
    if(Pvalue < 0.05) # pick Split
      whosRight = 'split'
    else
      whosRight = 'lump'
    return(
      list(Pvalue=Pvalue, lump=lump, split=split, confInterval_Split=confInterval_Split,
           confInterval_Lump=confInterval_Lump, whosRight=whosRight)
      )
}
testChoices <<-
  list("Fisher exact test"=fisher.test, "chi square test"=chisq.test)
output$fisherResult = renderUI({
  analysisName = 'Test_to_choose_who_is_right'
  source(analysisReactiveSetup_DTC, local=TRUE)

  
  if(is.null(input$chosentest))
    chosentest =fisher.test
  else
    chosentest = testChoices[[input$chosentest]]
  
  testresult = Test_to_choose_who_is_right(thisData, chosentest=chosentest)

  theText = paste(
    "If you change the data, <br>then the P value changes.<br>
      With ", strong(em(input$chosentest)), ",", br(), " the P value is  P = " ,
    strong(em(signif(digits=3, testresult$Pvalue))),
    ". <br>")
  if(testresult$Pvalue < 0.05) # pick Split
    {theText = paste(theText,
      "That's less than 0.05. <br> So the test concludes 
      <em><strong><font color='", splitColor, 
      "'>'Dr. Split is right'.  </em></strong></font><br> 
      We go with the Dr. Split estimate:<br><br>
      Our guess:  <em><strong>Pr(R|D) =  ", 
      testresult[['split']],
      '</em></strong>')
    }
  else{  # pick Lump
    theText = paste(theText,
      "That's greater than 0.05. <br>So the test concludes 
      <em><strong><font color='", lumpColor, 
      "'>'Dr. Lump is right'. </em></strong></font><br> 
      We go with the Dr. Lump estimate:<br><br>
      Our guess:  <em><strong>Pr(R|D) =  ", 
      testresult[['lump']],
      '</em></strong>')
  }
...
```{r fisherResultDTC}
jumpList_DTC = c(jumpList_DTC, Test_to_choose_who_is_right='Test to choose who is right')
source('analysisInitialSetup_DTC.R', local=TRUE); a(name=paste0('section-a_', 'Test_to_choose_who_is_right') )

fluidRow(
  column(7, br(),br(),br(),br(),br(),
         radioButtons(inputId ='chosentest', label = "which test to use", 
                 choices = names(testChoices), selected = "Fisher exact test"),
         uiOutput('fisherResult')),
  column(5,
         br(),
         dataTableComponent(analysisName='Test_to_choose_who_is_right')
))
...
### <a class='threesharps' name='section-Bayes_mixture'> APPROACH: Bayes mixture of smooth "Lump" and smooth "Split" prior beliefs</a> <!-- #title -->


```{r DrWho, child = 'DrWho.Rmd'}
```

```{r WhoNotation, child = 'DrWho_prior_notation.Rmd'}
```
...
### <a class='threesharps' name='section-Bayes_joint_prior'> APPROACH: Bayes joint prior, logit scale.</a> <!-- #title -->

No more crude mixture of the two extreme views of   
"Dr. Lump" and "Dr. Split".  
Another fancier Bayesian approach is to set a Bayesian prior  
on the joint distribution of the two conditional probabilities  
Pr(R | D) and Pr(R | L).  
This allows data on the two probabilities for $D$ and $L$ to be shared between them,  
smoothly, instead of the "Dr. Lump" versus "Dr. Split" winner-take-all.
The extremes of Lump and Split, and the compromise view of Who,  
are expressed by setting values for  

* the prior variance within group (phi)
* shared additional variance (tau)  

Dr. Split says there IS no shared variance:  tau=0.  
Dr. Lump says it's ALL shared variance:  tau = 1. (Well,  almost all; a tiny phi for nice plotting.)  

Dr.Who says, forget about you guys, I'm making my own prior!

My prior variance within group (phi) equals 0.5  
My prior shared additional variance (tau) equals 0.5  
My shared prior mean (mu)  is 0.5.  
So I, Dr. Who, present to you a Bayesian mango.


The phi and the tau are relevant on the **logit** scale. See below for the details about logits. You can adjust to get just the mango you want, to describe an initial belief and uncertainty you are confortable with.  

```{r QA-smooth-joint-prior}
QandAha('QA-smooth-joint-prior')
```
Once we have this joint prior distribution,  
we can compute the joint posterior distribution.


```{r Bivariate contour plot}
analysisName = 'Bivariate contour plot'
jumpList_DTC = c(jumpList_DTC, bivariateContourPlot='Bivariate contour plot')
source('analysisInitialSetup_DTC.R', local=TRUE); a(name=paste0('section-a_', analysisName) )

source('bivariateContourPlotReactive.R', local=TRUE)

output$contourTitles = renderUI(
  tagAppendAttributes(
    div(
    ...
    style=
      paste0('text-align:center; font-size:small;
                    font-weight:bold; color:',
             textOutput('colorForDoctorSelected'), 
             ';'
      ))
)
```

#### <a class='foursharps' name='section-Contour-parameters-and-graph'> The joint distribution contour plots</a> <!-- #title -->
...
div(
  fluidRow(
    ...
    column(8,
           uiOutput('contourTitles'),
           uiOutput('posteriorMean'),
           plotOutput(outputId = 'thePlot', width='100%'),
           br(),
           ContoursPanelLegend
    ))
...
The raw Lump and Split the estimates and  confidence intervals are shown.  
For Dr. Who, the estimate will be between Lump's and Split's,  
and the confidence interval will be  narrower than  Split's,  
because the $L$ data is contributing but not dictating.

To set the prior on the joint distribution of  
Pr(R | D) and Pr(R | L), we first convert them to  
logits ("logit" means "log(odds)").

```{r Details about the logit}
conditionalPanelWithCheckbox('Details about the logit', filename = 'www/logit.Rmd')
### yes it will render with mathjax!
###
```

#### <a class='foursharps' name='section-Contour-prior-posterior'> Calculating the posterior distribution: "log odds"</a> <!-- #title -->

Then we set our prior belief/uncertainty  distribution as a bivariate normal distribution on the logits of the two response probabilities.  

We also convert the observed proportions to logits.  
We estimate the variances using the **delta method**.  
That gives us confidence intervals.

Details on the **delta method** are here:
```{r}
...
We can apply the delta method to the Poisson distribution,  
a distribution very useful for count data like the data in our table.  
The variance of the logarithm of a count is roughly  
the reciprocal of the count.

(This doesn't work well if the count is near zero,  
and not at all at exactly zero.
For this reason a "continuity fudge factor" is one of our parameters,
necessary around the edges.)
...
###  <a class='threesharps' name='section-Cross_validation'>APPROACH: Optimizing the mixture with cross-validation </a><!-- #title -->

Cross-validation is a technique in which  
the performance of a model can be checked  
as if on an independent dataset,  
without actually having a separate dataset.  
*$K$-fold cross-validation* divides the dataset into  
K disjoint subsets of observations,  
and loops over these sets.  
Each subset is, in turn, used as a `test set`. 
We remove it from the full dataset,  
re-build the model on the reduced dataset (the `training set`),  
and evaluate predictions of that model  
on the removed observations in the test set.  
Performance metrics are usually averaged across the $K$ repetitions.  
When $K=N$, the size of the full dataset,  
the procedure is called `leave-one-out cross-validation`,  
since each test set is of size one.

For this example dataset,  
we perform leave-one-out cross-validation, with a "LEAST SQUARES" penalty:  
* If the one left out is a *responder*,  
$penalty$ =  $(1-prediction)^2$.  
* If the one left out is a *non-responder*,  
 $penalty$ =  $prediction^2$.  

where the $prediction$ is based on the rest of the sample, using the weights for $Lump$ and $Split$.  
(Other penalty functions are sometimes used.)  
(We add penalties over the four cells, not each patient.
One could also weight these penalties by the number in each cell, heavily weighting the Pr(N|L) cell.  Or even omit the two L cells.)
...
div(
   fluidRow(
    column(6, br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),
           radioButtons('penaltychoice', label = 'penalty function', 
                        selected = names(penaltyFunctions)[1],
              choices = names(penaltyFunctions))),
    column(6, plotOutput('penaltyPlot'))
    ),
  fluidRow(
    column(5, 
...
Peek ahead by opening up ***"Jump to other analyses"***   
to see how these optimal weights compare to the other methods.  

For the original data, the optimal weighting is at 0.73 for Split (& 0.27 for Lump). 
This achieves the lowest average penalty.  
The predicted probability of response Pr(R|D)  is 38%.  
Not as good as 50%, but a lot better than 7%.  

So this is another way of "borrowing strength" from the larger subgroup.
...
TextQuestion("This counts penalties for all patients. Should it only include penalties for the predictions for the D people?")
TextQuestion("What other flaws does this method have?")
QandAha('QA-for-crossvalidation')
```


### SUMMARY of APPROACHES: table of estimates and weights  <!-- #title -->

Assembling the Posterior probabilities and weights  
for the approaches to estimating  
Pr(R | D):

```{r tableOfEstimates}
jumpList_DTC = c(jumpList_DTC, estimateTable='SUMMARY of APPROACHES')
source('analysisInitialSetup_DTC.R', local=TRUE); a(name=paste0('section-a_', analysisName) )

output$tableOfEstimates = 
  renderDataTable( server=FALSE, { #rownames = TRUE, {
    #server =FALSE    allows the navigation stuff to be hidden.
    #reloadData() only works for tables in the server-side processing mode, e.g. tables rendered with renderDataTable(server = TRUE)
    analysisName = 'estimateTable'
    
    source(analysisReactiveSetup_DTC, local=TRUE);
      ###  Use the last-data for everything.

    debuggingWithModals = FALSE
       
    #isolate  # removing this isolate call doesn't help.
    ( {
      #cat(paste(thisData), '\n')
    ...
    nN=sum(thisData['N', ])
    ## Raw results
    LumpRawMean=(nR)/(nR+nN)
    SplitRawMean=(nRD)/(nRD+nND)
    #print(thisData)
    #cat(nN, ' ', nR, ' ', LumpRawMean, '\n')
    ...
    pProbSplit = posteriorProb(rValues$WhoPriorProb, theData=thisData)
    ...
    if(debuggingWithModals) 
      showModal(modalDialog(      'after CV'    ))

    ### Bivariate logit method results.
    #setDLdata(value=thisData, analysisName='bivariateContourPlot')
    
    # if n_RD is zero, anomalous weights. Possible delta method failure.
    
    paste(   'after bivariateNormResults',
    ...
    # ##### goofy fix for initial "non-conformable arrays" error.
    # if(class(bivariateNormResults) == "try-error") {
    #   #if(debuggingWithModals) 
    #     #showModal(modalDialog(   ### we need this one!
    #       paste(   'after bivariateNormResults',
    #                input$tauInput, input$phiInput, input$mu0Input )    #))
    #   #updateNumericInput(inputId="mRDidPanelDTC5",value=6)
    ...
    # print('implicitWeight1_ForLogit')
    # print(implicitWeight1_ForLogit)
    ...
    testresult = Test_to_choose_who_is_right(thisData)
    printpaste('testresult', testresult)
    testresult$selectedMean = ifelse(testresult$whosRight=='lump',
                                   testresult$lump, testresult$split)
    means = rValues$means = c(
        LumpRawMean,
        SplitRawMean,
        testresult$selectedMean,
        #rValues$estimates_choose_Lump_or_Split[[rValues$whosRight]],
        posterior.mean.w,
        bivariateNormResults$postmean.p[1],
        CVoptimalEstimate
      )
      weightsForLump = rValues$weightsForLump = c(
        1,
        0,
        c(lump=1, split=0)[testresult$whosRight],
        pProbLump,
        implicitWeights_ForLogit[2],
        1-CVoptimalWeight
      )
      weightsForSplit = rValues$weightsForSplit = c(
        0,
        1,
        c(lump=0, split=1)[testresult$whosRight],
        pProbSplit,
        implicitWeights_ForLogit[1],
        CVoptimalWeight
      )
      CVpenalties = sapply( rValues$weightsForSplit, function(w)
        doCVanalysis(thisData, w)[['penaltyAtOpt']])
      #if(debuggingWithModals) 
      # showModal(modalDialog(title = 'CVpenalties',
      #                       CVpenalties)   
      # )
      # print(CVpenalties)
  ...
  whosRightCapitalized <<- c(lump="Lump", split="Split", who="Who")
  
  result = data.frame(means, weightsForSplit, weightsForLump, CVpenalties)
  result = as.data.frame(lapply(result, round, digits=3) )
  rownames(result) = c(
    'Raw proportion (Lump)',
    'Raw proportion (Split)',
     paste('Test chooses ', whosRightCapitalized[testresult$whosRight]),
    #span(style='color:red', 
   #      paste('Test chooses ',
    #           whosRightCapitalized[testresult$whosRight])   , #),
    #paste('Test chooses ', rValues$whosRight),
  ...
  # print(result)   # don't print the table
  rValues$newResultTable = result
  # x = kableExtra::kbl(result) # %>%
  #            #kable_styling(full_width = F)
  # (print(x))
   x = DT::datatable(result,  autoHideNavigation = T, 
            options=list(buttons=F,pageLength=nrow(result)))
   #   print(rownames(x))   NULL.
   #   x = DT::
   testresultColor =  ifelse(testresult$whosRight=='lump',
                             "red", "blue")
   # print(paste('testresultColor', testresultColor))
   x = formatStyle(x, columns = 0:5, 
                   fontWeight=styleRow(6, 'bold'), #border=5,
                   color=styleRow(1:5, 
                                  values=c('red','blue',
                                          testresultColor,
                                          'green','green')),  
   ...
   isolate({
     # if(! is.null(rValues$savedTable)){
     #   entriesChanged =
     #     (rValues$savedTable != result)
     #   print('entriesChanged:')
     #   print(table(entriesChanged))
     #   for(irow in 1:nrow(result))
     #     for(icol in 1:ncol(result)) {
     #       if(rValues$savedTable[irow,icol]
     #          != result[irow,icol]){
     #         print(paste('entriesChanged', irow, ', ', icol, 
     #                     ' from ', rValues$savedTable[irow,icol], 
     #                     'to', result[irow,icol]))
     #         x = formatStyle(x, columns = icol, 
     #                         fontWeight=styleRow(irow, 'bold'), #border=5,
     #                         color=styleRow(irow, values=c( color='purple'))  
     #                         , target='cell')
     #       }
     #     }
     #   #   wrapCSS(x [entriesChanged], "color:blue")
     #   # x[1] = gsub("&gt;", '>', x[1])
     #   # x[1] = gsub("&lt;", '<', x[1])
     # }
     rValues$savedTable = result  # no css formatting
     rValues$savedTableX = x  # 
   })
  ...
  rValues$proxy = dataTableProxy('tableOfEstimates')
# this works: selectRows(rValues$proxy, 2)
  # Not this:  selectCells(rValues$proxy, selected = matrix(c(1:4),nrow=2) )
  
 #    observeEvent(input$tableOfEstimates,{
 #   if(! is.null(rValues$savedTable)){
 #     rValues$entriesChanged = entriesChanged =
 #       (rValues$savedTable != result)
 #     print(paste('entriesChanged:', table(entriesChanged)))
 #     # x [entriesChanged] =
 #     #   wrapCSS(x [entriesChanged], "color:blue")
 #     # print(x)
 #     # x[1] = gsub("&gt;", '>', x[1])
...
div(style='fontsize:large;', 
    dataTableOutput('tableOfEstimates'),
    WhoPriorProbNumericInput(),
    dataTableComponent(analysisName = 'estimateTable')
)
```

As you change Dr. Who's prior probability or the data values,  
observe the changes in this summary table.  
Raw (Lump) & Raw (Split)	are the table proportions  
taken at face value  by Dr. Lump and Dr. Split.  
Bayes Mean (Lump)	and Bayes Mean (Split) and Bayes Mean (Who)  
are Bayesian posterior means that we calculated above.  
Bayes Pr(Lump) and	Bayes Pr(Split) are  
Dr. Who's posterior probabilities that  
Dr. Lump or Dr. Split (respectively) is correct.
...
## Parametrizing the probabilities: likelihood functions <!-- #title -->

We must dive deeper. We need some notation.  
"\*Yawn\*"?   NO!  Exciting. Good notation is POWERFUL.

<!-- #### parametrize.Rmd #### -->
```{r parametrize.Rmd, child = 'Rmd-text-snippets/parametrize.Rmd'}
```
...
So, when should we lump? When should we split?  
Should we aim to compromise, and if so,  
how should we strike the balance?  

In this section we explore different scenarios  
of external information, to see how they affect the estimation of ${Pr(R|D}$.


##	<a class='threesharps' name='section-Strong_prior_beliefs'>Strong prior beliefs about $X_{DL}$ </a><!-- #title -->

### <a class='threesharps' name='section-featureImplausible'>Strong prior belief that feature $X_{DL}$ is ***IMPLAUSIBLE*** </a><!-- #title -->


What if D and L represent ***dark and light hair color***?  
"Dr. Who" may be **highly skeptical** that this $X_{DL}$
really affects response.  
Then "Dr. Who" may choose a prior odds  
that favors ***Dr. Lump***  over Dr. Split.  
How does that matter?  
Revisit the prior section: `r go_to_section()`



###	<a class='threesharps' name='section-featureImplausible'>Strong prior belief that the feature $X_{DL}$ is ***IMPORTANT*** </a> <!-- #title -->

But what if D and L represent two variants of a  
genotype shown to affect this drug's **pharmacodynamics**?  
  
Or, maybe  the D folks and the L folks got different isoforms of the medicine ?  
The **D** folks got the **Dextro** form of the  drug, the **L** folks the **Levo** drug?

<!--
Or perhaps, $X_{DL} = D = dark hair$, $X_{DL} = L = light hair$,  
but hair color is strongly tied to ethnicity in this sample  
… which is strongly tied to a key genetic variant.  
-->

Then "Dr. Who" may choose a prior odds  
that favors ***Dr. Split***  over Dr.  Lump.  
How does that matter?  
Revisit the prior section 
`r go_to_section() `
...
Which of the previous analyses allow you to  
interject prior belief about the credibility of Lump versus Split?

##	Multiple features, with some prior belief <!-- #title -->

Suppose that we have features for a *hundred genes*  
which were *previously* identified as known to affect the  
pharmacodynamics of the drug being tested.  
Our feature $X_{DL}$ happens to be one of them;  
...
As before, suppose that we have features for a hundred genes  
which were *previously* identified as known to affect  
the pharmacodynamics of the drug being tested.  
But this time, our feature $X_{DL}$ is selected  
to be the best of the one hundred features.

This situation entails dealing with *"multiple comparisons"*.  
Pure chance can make a feature seem strongly associated with the response outcome when in reality it is not associated.  
When the best-appearing relationship is selected out of a large number,  
we can be fooled into thinking the strength of evidence  
...
One way to handle multiple comparisons is through a  
classical frequentist *multiple testing adjustment*.  
This evaluates the chance of getting a P-value as low as  
(or lower than) the minimum of the P-values for the features.  
The best-known of these methods are the  
Bonferroni, Sidak, and permutation /randomization tests.  
...
and here are some examples that highlight some  
strange and unsettling aspects of the classical methods:
...
What if we add lots of features to the feature  
($X_{DL}$) of our little data set?  
How will the methods we have used behave when applied in this "big" data set.
Let's refer to $X_{DL}$ as $X_1$,  
and let's generate 99 extra features $X_2 ... X_{100}$ ( 
each with only two possible values, 0 or 1).
...
checkYesNoYes = function(seed, qStar=1) {
    ...
    allChisqPs = generateAllPvalues(
      makeBigDataWithFeatures(seed=seed, whoFrom='generateAllPvalues') )$allChisqPs
...
makeBigDataWithFeatures = function(DLdata = DLdataOriginal,
                                  ...
                                  Omega = 0,
                                  whoFrom = 'whoFrom unknown',
                                  seed){
  if(printBDCProgress)
    print(paste('makeBigDataWithFeatures: whoFrom: ', whoFrom))
...
## Combine DL feature with extra features. 
  ...
  if(printBDCProgress)
    print(paste('makeBigDataWithFeatures:', dim(DLdataWithFeatures),
                ' whoFrom: ', whoFrom))
...
generateAllPvalues = function(
  BigData = makeBigDataWithFeatures(whoFrom='generateAllPvalues')) {
  allFisherPs <<- 
...
BigDataOriginal = makeBigDataWithFeatures(DLdataOriginal, seed=45, whoFrom='BigDataOriginal')
...
Now that we have a data set with lots of features,  
let's do Fisher exact tests and chisquare tests  
to all the features.  
The red "X" is of course our original data feature,  
with $X_{DL}$ = D or L.
...
For each of these features $X_2 ... X_{100}$,  
the disparity between the two groups $X_j = 0$ and $X_j = 1$  
can be represented as the odds ratio:

\[\frac{{odds(Y = R|{X_j} = 0)}}{{odds(Y = R|{X_j} = 1)}} = \frac{{\Pr (Y = R|{X_j} = 0)/\Pr (Y = N|{X_j} = 0)}}{{\Pr (Y = R|{X_j} = 1)/\Pr (Y = N|{X_j} = 1)}}\]

Initially the extra 99 features are all "null hypotheses".  
The odds ratios all equal 1,  
so the odds that $Y=R$ does not change with $X_j$.  
But we can explore what happens if they do not all equal 1.
You can control Omega, the variance of logs of the odds ratios below.  
When Omega equals zero, the odds ratios all equal 1.  
Otherwise, they will spread out, some bigger than 1 and some smaller.
...
Applying the *"Bonferroni correction"* to the Fisher P value  
for our feature $X_{DL}$ means multiplying the  
P value `r signif(digits=4, allFisherPs_original[1])`  
by 100, to get P=`r signif(digits=4, 100* allFisherPs_original[1])`.  
So the evidence that the `D` people really are different is minimal at best.  
(To say it carefully, if a 'cutoff' for defining 'statistically significant'  
happened to be chosen at `r signif(digits=4, 100* allFisherPs_original[1])`,  
then our result for $X_{DL}$ would be just on the  
edge of 'significance'.  
That's not a compelling cutoff!) 

In the classical statistics spirit,  
we would now go with Dr. Lump instead of Dr. Split.  
(Choosing instead the chisquare test, the Bonferroni correction gives us P=`r signif(digits=4, 100* allChisqPs_original[1])`).
...
fun.BHtable = function(){
  ...
  if(inherits(tryResult, "try-error")
     | is.null(tryResult) |is.na(tryResult)) {
    qStar = 0
...
We can apply EB to a collection of estimates.  
Let's collect the estimates of association 
between the outcome and all 100 features. 
These estimates are the odds ratios. 
For the original feature $X_{DL}$, the odds ratio is  
`r DLdataOriginal['R','D']` / `r DLdataOriginal['N','D']` * `r DLdataOriginal['R','L']` / `r DLdataOriginal['N','L']`, which equals 
***`r oddsRatioOriginal = DLdataOriginal['R','D'] / DLdataOriginal['N','D'] * DLdataOriginal['R','L'] / DLdataOriginal['N','L'];  signif(digits=2, oddsRatioOriginal)` .***
...
Using the delta method described in a previous section, 
...
```{r bigDataComponent analysisName}
bigDataComponent(analysisName=analysisName)
```
