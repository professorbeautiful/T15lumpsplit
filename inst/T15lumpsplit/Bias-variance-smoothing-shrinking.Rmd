...
```{r}
observeEvent(rValues$DLdata, {
  DLdataDF = 
    data.frame(
      outcome = rep(c("R","N", "R","N"),
                    times=c(rValues$DLdata)),
      X_DL = rep(c("D","D", "L","L"), times=c(rValues$DLdata))
    )
  NextraFeatures = 99
  ...
  extraFeatures = as.data.frame(
    lapply(extraFeaturesProbs,
           rbinom, n=sum(rValues$DLdata),size=1)
  )
  names(extraFeatures) = paste0('X', 1+(1:NextraFeatures))
  rValues$DLdataDF = cbind(DLdataDF, extraFeatures)
})
...
```{r}
allFisherPs = sapply(DLdataDF[-1], function(feature)
  fisher.test(feature, DLdataDF$outcome)$p.value)
hist(allFisherPs)
#head(sort(allFisherPs))
options.saved = options(warn=-1)
allChisqPs = sapply(DLdataDF[-1], function(feature)
  chisq.test(feature, DLdataDF$outcome)$p.value)
hist(allChisqPs)
#head(sort(allChisqPs))
plot(allChisqPs, allFisherPs, log='xy')
points(allChisqPs[1], allFisherPs[1], pch='X', col='red')
options(options.saved)
```

Applying the *"Bonferroni correction"* to the Fisher P value for our feature `X` means multiplying the P value `r signif(digits=4, allFisherPs[1])` by 100, to get P=`r signif(digits=4, 100* allFisherPs[1])`. So the evidence that the `D` people really are different is minimal at best. (To say it carefully, if a 'cutoff' for defining 'statistically significant' happened to be chosen at `r signif(digits=4, 100* allFisherPs[1])`, then our result for `X` would be just on the edge of 'significance'. That's not a compelling cutoff!) 

In the classical statistics spirit, we would now go with Dr. Lump instead of Dr. Split. (Choosing instead the chisquare test, the Bonferroni correction gives us P=`r signif(digits=4, 100* allChisqPs[1])`).
...
output$BHtable = renderTable({
  ...
  somePvalues  = signif(digits=3, sort(allChisqPs)[1:8])
  BHtable = data.frame(
    Pvalue = somePvalues,
    cutoff = qStar * (1:8)/100)
  BHtable[[ 'P<cutoff' ]] = c('no','yes')[
    1 + (BHtable$cutoff > BHtable$Pvalue) ]
  BHtable = t(BHtable)
  rownames(BHtable) = c('Pvalue', 'cutoff', 'signif')
  BHtable
})
...
```{r}
TextQuestion("which features are 'significant'?")
```

Raising qStar from 1.0 through 1.5,

```{r}
TextQuestion("how does the collection of 'significant' features change?")
```
...
We can apply EB to a collection of estimates. In this instance, let's collect the estimates of association between the outcome and all 100 features. These estimates are the odds ratios. For the original feature `X`, the odds ratio is `r DLdataOriginal['R','D']` / `r DLdataOriginal['N','D']` * `r DLdataOriginal['R','L']` / `r DLdataOriginal['N','L']`), which is 
`r DLdataOriginal['R','D'] / DLdataOriginal['N','D'] * DLdataOriginal['R','L'] / DLdataOriginal['N','L'] ` ).
...
To apply the EB idea, we "shrink" all the logit ratios towards the common mean, using a model in which the true logit ratios are all drawn from a common parent distribution, assumed to be normal (Gaussian). Then a standard Bayesian calculation gives us the posterior means (and variances) for each logit ratio. But we would have to know the true mean and variance of the parent distribution. The "emprical" part of empirical Bayes is to use the data to estimate the mean and variance of the parent.

From the marginal distribution of the observed logit ratios, we get
