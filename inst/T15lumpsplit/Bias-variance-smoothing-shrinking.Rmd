...
#  linktext = paste0('▸ ', jumpList_DTC[[sectionName]]) 
...
output$fisherResult = renderUI({
  ...
  rValues$estimates_choose_Lump_or_Split = list(
    lump = round(digits=2, sum(thisData['R', ])/
      sum(thisData[ , ])),
    split =  round(digits=2, (thisData['R', 'D'])/
      sum(thisData[ , 'D'])),
  ...
  if(Pvalue < 0.05) # pick Split
    {theText = paste(theText,
      ...
      rValues$estimates_choose_Lump_or_Split[['split']],
      '</em></strong>')
    rValues$whosRight = 'split'
    }
  else{  # pick Lump
    theText = paste(theText,
      ...
      Our guess:  <em><strong>Pr(R|D) =  ", 
      rValues$estimates_choose_Lump_or_Split[['lump']],
      '</em></strong>')
    rValues$whosRight = 'lump'
  }
...
output$tableOfEstimates = 
  renderTable(rownames = TRUE, {
    ...
    niceColHeader = c(
      ...
      weightsForLump = rValues$weightsForLump = c(
        1,
        0,
        c(lump=1, split=0)[rValues$whosRight],
        pProbLump,
        1-CVoptimalWeight,
      ...
      weightsForSplit = rValues$weightsForLump = c(
        0,
        1,
        c(lump=0, split=1)[rValues$whosRight],
        pProbSplit,
        CVoptimalWeight,
  ...
  whosRightCapitalized <<- c(lump="Lump", split="Split", who="Who")
    ...
    rownames(result) = c(
      ...
      paste('Test chooses ', whosRightCapitalized[rValues$whosRight]),
