...
checkYesNoYes = function(seed, qStar=1) {
    ...
    allChisqPs = generateAllPvalues(
      makeDLdataWithFeatures(seed=seed) )$allChisqPs
...
makeDLdataWithFeatures = function(DLdata=DLdataOriginal,
  ...
  if( abs(tauTrue) < 1e-9) {
    extraFeaturesProbs = extraFeaturesProbs_R
...
generateAllPvalues = function(
  DLdataWithFeatures = makeDLdataWithFeatures()) {
  allFisherPs <<- 
...
observeEvent(list(input$regenerateFeatures, input$ORvarianceID), {
    try( {
      tauTrue = as.numeric(input$ORvarianceID) 
      cat('tauTrue:', tauTrue, '\n')
...
div(
  ...
  numericInput('ORvarianceID', 'Variance of the Odds Ratio ',
...
observeEvent(
       ...
       input$qStarID),
  {
     try({
      ...
      # qStar = as.numeric(input$qStarID)
...
output$empBayeslogitRatios = renderPlot({
  ...
  rValues$tauEstimateEB = tau
  cat('mu-hat: ', mu, '  tau-hat: ', tau, '\n')
  weights = 1 - tau / (rValues$logoddsratiovariances + tau)
...
The estimated variance is `r output$tauEstEB = renderText(rValues$tauEstimateEB); textOutput('tauEstEB', inline=TRUE)`. (The value that generated the data is `r output$tauTrue = renderText(input$ORvarianceID); textOutput('tauTrue', inline=TRUE)`) .
