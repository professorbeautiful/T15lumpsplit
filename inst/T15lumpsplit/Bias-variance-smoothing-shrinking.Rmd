...
```{r Overview}
h2("NOTE: this is a work in progress!")
...
Of them, only 8 respond.  But there is a subgroup of 5 in which 3 patients respond, yielding a response rate of 60%. for now we call them "D patients", and the others "L patients". Which subgroup a patient belongs to we will call the person's "feature" $X$, with possible values $D$ or $L$.
...
If you *"lump"*, ignoring the group, you will estimate that the response rate (`Pr(R|D)`) for D patients is the same as `Pr(R)`, utilizing the whole set of patients, which is  
`r reactive(round(100*estimates()$lump))`*%*. 
...
div(br(),br(),br())
```
You can change the numbers in the table and observe the results. (Be sure to reset them when done.)

Lumping gives an answer with low variance ($N$=100) but high bias-- because the answer reflects far more $L$ patients than $D$ patients. The confidence interval follows the diagonal where $Pr(R|D)=Pr(R|L)$.

Splitting gives an answer with high variance ($N$=5) but low bias-- because it is including only patients who ARE in the $D$ group:  it is directly asking our question. The very wide horizontal confidence interval reflects this high variance.
...
If the P-value is "significant", the "null hypothesis" (that "Dr. Lump" is correct) is rejected. With the original data, the traditional consequence is that we use Dr. Split's estimate:  60%...  and WE COMPLETELY IGNORE THE DATA FOR THE "L" PEOPLE!
...
Another classical approach, the Benjamini-Hochberg method, is widely used in high-throughput biological assays. It has a technical advantage in that the stated "false discovery rate" is accurate. However, it has peculiar and disturbing behavior:
```{r}
conditionalPanelWithCheckboxPDF(
...
One popular alternative with an empirical Bayes flavor is the "Qvalue" method. 
