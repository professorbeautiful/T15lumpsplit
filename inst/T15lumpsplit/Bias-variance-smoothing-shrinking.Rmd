...
output$oddsratioplot = renderPlot({
  ...
  jitteredLogits = rValues$jitteredLogits = 
    logoddsratios + runif(length(logoddsratios), c(-1,1)*diff(range(logoddsratios))/500)
  print(table(table(jitteredLogits)))
  plot(density(jitteredLogits), xlab='log(oddsratios)',
...
(The tall red lines are just multiple observations.)
...
```{r}
output$tauEstEB = renderText(rValues$tauEstimateEB)
thisTauTrueID = get_thisTauTrueID(mapAnalysisToBDCnumber[analysisName])
thisTauTrueID_text = paste0(thisTauTrueID, '_text')
output[[thisTauTrueID_text]] = renderText(input[[thisTauTrueID]])
```

The estimated variance is `r  textOutput('tauEstEB', inline=TRUE)`. (The value that generated the data is `r  textOutput(thisTauTrueID, inline=TRUE)` ) .
