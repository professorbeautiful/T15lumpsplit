...
output$fisherResult = renderText({
    Pvalue = fisher.test(rValues$DLdata[[1]])$p.value
    paste0(
...
output$plotLumpSplitPoints = renderPlot(
  height = 300, {
  plotPlightPdarkPosterior(DLdata=rValues$DLdata[[analysisNumber]],
                           showPrior = FALSE, 
...
```{r BayesLogitPlot}
jumpList = c(jumpList, viewPlot='Bivariate logit plot')
source('analysisInitialSetup.R', local=TRUE)
...
observeEvent(list(input$regenerateFeatures, rValues$DLdata), {
