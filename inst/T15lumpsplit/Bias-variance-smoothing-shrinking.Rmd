...
analysisReactiveSetup_options = paste0(c('inst/T15lumpsplit/', '',  'T15lumpsplit/'),'analysisReactiveSetup.R')
whichExist = print(sapply(analysisReactiveSetup_options, file.exists))
analysisReactiveSetup = print(analysisReactiveSetup_options[whichExist][1])
```
...
#cat("After first analysisInitialSetup.R, getDTCnumber('plotLumpSplitPoints') =",
#    getDTCnumber('plotLumpSplitPoints'), '\n' )
estimates = observeEvent( getDLdata('plotLumpSplitPoints'),
  {
...
output$plotLumpSplitPoints = renderPlot(
  height = 300, {
    ...
    plotPlightPdarkPosterior(
      DLdata=getDLdata('plotLumpSplitPoints'),
      showPrior = FALSE, 
...
(Throughout this module, you can change the numbers in the table and observe the results. 
Be sure to press "reset" when done.)

If you *"lump"*, ignoring the group, you will estimate that the response rate (`Pr(R|D)`) for $D$ patients is the same as `Pr(R)`, utilizing the whole set of patients, which is `r reactive(round(100*rValues$estimates$lump))`*%*. 

If you *"split"*, you will estimate that the response rate (`Pr(R|D)`), utilizing the D patients only, is 
`r reactive(round(100*rValues$estimates$split))`*%*. 
...
fluidRow(
  column(4, 
         ...
         span(style='text-align:center;font-weight: bold', 
              "Confidence intervals for Pr(R|D):"),
...
Lumping gives an answer with low variance ($N$=100; the sample size is fairly high) but high bias-- because the answer reflects far more $L$ patients than $D$ patients. So we get a precise answer to what might be the wrong question. The confidence interval follows the diagonal of the plot on the left above, where $Pr(R|D)=Pr(R|L)$.
...
One test frequently performed to test independence in a 2x2 table like this one is the *Fisher exact test*. On the original data, the P value is `r textOutput('fisherResultOriginal', inline=TRUE)`. If you have changed the data in the table above, it is now `r textOutput('fisherResult', inline=TRUE)`.
...
jumpList = c(jumpList, crossvalidationPlot='Cross-validation plot')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
...
```{r BayesLogitPlot}
jumpList = c(jumpList, bivariateContourPlot='Bivariate contour plot')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
...
```{r tableOfEstimates}
jumpList = c(jumpList, estimateTable='Table of estimates')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )

output$BayesPostProbs = 
  renderTable(rownames = TRUE, {
    ...
    implicitWeight1_ForLogit = 
      (rValues$bivariateNormResults$postmean.p[1] - LumpRawMean)/
      (SplitRawMean - LumpRawMean)
    implicitWeights_ForLogit = 
      round(digits=2, c(implicitWeight1_ForLogit, 1-implicitWeight1_ForLogit) )
    # niceColHeader = c( 
    #   'Bayes Pr(Lump)',
    #   'Bayes Pr(Split)',
    #   'CV weight (Lump)',
    #   'CV weight (Split)'
    # )
    setDLdata(value=thisData,
              analysisName='crossvalidationPlot')
    #rValues[[paste0('DLdata', currentDTCnumber) ]]
    CVoptimalWeight = doCVanalysis()$optimalWeight
    result = data.frame(
      c(pProbLump, round(1-CVoptimalWeight, digits=2)),
      c(pProbSplit, CVoptimalWeight),
      implicitWeights_ForLogit
    )
    names(result) = c("Lump", "Split")
    rownames(result) = c("Dr.Who's posterior probabilities", 
                         "Cross-validation weights", 'Bivariate logit weights')
    return( (result) )
  })


output$estimateTable1 = 
  renderTable({
  ###  Use the last-data for everything.
  ### Data for the full table... hmm, how to keep this from reacting?
  ### For now just use DLdataOriginal
  analysisName = 'estimateTable'
  source(analysisReactiveSetup, local=TRUE);
  # analysisReactiveSetup.R: Table of estimates   currentDTCnumber= NA 

  isolate( {
    cat(paste(thisData), '\n')
    cat('Copying data to other analyses:\n')
    for(aN in 1:analysisNumber) {
      cat('  =>',  names(mapAnalysisToDTCnumber) [aN], ':  ')
      setDLdata(value=thisData, myChoice=TRUE, names(mapAnalysisToDTCnumber) [aN] )
      cat(paste(getDLdata(myChoice=TRUE, names(mapAnalysisToDTCnumber) [aN]) 
      ), '\n' )
    }
  })
  cat('\n')
  ...
  setDLdata(value=thisData,
            analysisName='crossvalidationPlot')
  #rValues[[paste0('DLdata', currentDTCnumber) ]]
  CVoptimalEstimate = doCVanalysis()$CVoptimalEstimate
  cat('CVoptimalEstimate', CVoptimalEstimate, '  ', paste(DLdataLastUsed), '\n')
  result = 
    data.frame(
      posterior.mean.l,  
      posterior.mean.s,  
      posterior.mean.w,  
      CV=CVoptimalEstimate
    )
  names(result) = niceColHeader
  return(result)
})

output$estimateTable2 = 
  renderTable({
  ###  Use the last-data for everything.
  ### Data for the full table... hmm, how to keep this from reacting?
  ### For now just use DLdataOriginal
...
div(style='fontsize:large;', 
    div(h4("Posterior probabilities and weights"),
        tableOutput('BayesPostProbs') ),
    div(h4("Estimated Means of Pr(R | D)"),
        fluidRow(column(8, tableOutput('estimateTable1') ),
             column(4, tableOutput('estimateTable2') ) ) ),
...
makeDLdataDFYesNoYes = function(DLdata=DLdataOriginal, qStar=1){
  isYesNoYes = FALSE
  seedInt = 44
...
makeDLdataWithFeatures = function(DLdata=DLdataOriginal,
                                  ## Make extra features.
...
## Combine DL feature with extra features. 
  return(cbind(DLdataDF, extraFeatures ) )
}
## generateAllPvalues.
generateAllPvalues = function(DLdataWithFeatures = makeDLdataWithFeatures()) {
  allFisherPs <<- 
    sapply(DLdataWithFeatures[-1], function(feature) 
      fisher.test(feature,
                  DLdataWithFeatures$outcome)$p.value
    )
  options.saved = options(warn=-1)
  allChisqPs <<- 
    sapply(DLdataWithFeatures[-1], ## removing the outcome column 
           function(feature) {
             result = chisq.test(feature,
                        DLdataWithFeatures$outcome)
...
DLdataDFwithFeatures_original = makeDLdataWithFeatures(DLdataOriginal)
...
observeEvent(list(input$regenerateFeatures), {
    isolate( {
...
observeEvent(rValues$DLdataDFwithFeatures, {
             Pvalues = generateAllPvalues(
               DLdataWithFeatures = rValues$DLdataDFwithFeatures
             )
             rValues$allChisqPs = Pvalues$allChisqPs
             rValues$allFisherPs = Pvalues$allFisherPs
})
...
```{r pvaluesPlots}
jumpList = c(jumpList, pvaluesPlots='P-value plots')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
```
...
jumpList = c(jumpList, BHtable='Benjamini-Hochberg')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )

numericInput('qStarID', 'qStar',
             value = 1, min=0,  step=0.1)
observeEvent(input$qstarID, {
  rValues$qStar = input$qStarID
})

observeEvent(
  list(rValues$allChisqPs,
       rValues$qStar),
  {
    rValues$BHtable = makeBHtable(rValues$allChisqPs, qStar=1)
    #rValues$qStar ????  )
  }
)
output$BHtable = renderTable(
  digits=3,  expr=rValues$BHtable
)
...
```{r oddsratioplot}
jumpList = c(jumpList, oddsratioplot='Odds ratio plot')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
...
output$oddsratioplot = renderPlot({
  ...
  outcome = rValues$DLdataDFwithFeatures$outcome
  ...
  oddsratios = sapply(rValues$DLdataDFwithFeatures[-(1)], oddsratio)
  rValues$logoddsratios = 
    logoddsratios = log(oddsratios)
  plot(density(logoddsratios), xlab='log(oddsratios)',
...
`r {ci=signif(digits=3, exp ( log(oddRatioOriginal) + c(-1,1)*sqrt(sum(1/DLdataOriginal))*qnorm(0.975))); paste(ci, collapse=' to ')}`.
...
jumpList = c(jumpList, empBayeslogitRatios='Empirical Bayes logit ratios')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
...
```{r qvaluePlot}
jumpList = c(jumpList, qValue='Qvalue adjustment')
source('analysisInitialSetup.R', local=TRUE); a(name=paste0('section-a_', analysisName) )
...
output$qvaluePlot = renderPlot({
  ...
  qout.orig  =
    qvalue::qvalue(c(allChisqPs_original[1], rValues$allChisqPs[-1]) )
  rValues$P_Q_for_X_DL_original = 
    c(qout.orig$pvalues[1], qout.orig$qvalues[1])
  qout.mod  =
    qvalue::qvalue(rValues$allChisqPs )
  rValues$P_Q_for_X_DL_modified = 
  ...
  aspect = diff(par("usr")[2:1])/diff(par("usr")[4:3])
  bonfTilt = atan(nFeatures * aspect)* 180/pi
  diagTilt = atan( aspect )* 180/pi
  text(0.00025, 0.00025 * nFeatures, 'Bonferroni', cex=2, pos=3, # 3 = above
       srt=bonfTilt, col='darkred')
  abline(a=0, b=1, col='blue')
  text(0.003, 0.003, 'diagonal', cex=2, pos=3, # 3 = above
       srt=diagTilt, col='blue')
...
As before, modify the data here to see how the Q-value changes. 

```{r}
dataTableComponent(showhide='hide', analysisName='qValue')
...
```{r Q-qvalues-changing-extra-features}
