...
```{r shinyjs, context="server", echo=FALSE, results='hide', include=FALSE}

#shiny::addResourcePath("shinyjs", system.file("srcjs", package = "shinyjs"))
#library(shinyjs)
invisible( {
  require(shinyjs);
  shinyjs::useShinyjs(rmd=TRUE, debug=TRUE, html=FALSE)
})
...
observe(x = {
  ...
  isolate ({
    for(thisTQNumber in seq(along=TQ_contexts)) {
      outputIdThisTQ = paste0('TQ', thisTQNumber)
      textareaIdThisTQ = paste0('id', outputIdThisTQ)
      currentValue = as.vector(input[[textareaIdThisTQ]])
      cat("outputIdThisTQ", outputIdThisTQ, '==>',
          currentValue, '<==\n')
#      if( !is.na(currentValue) ) {
        cat('Filling ...',
            cookieValues[outputIdThisTQ], '\n')
        updateTextAreaInput(
          session,
          textareaIdThisTQ,
          value =
...
#      }
    ...
    for(thisQANumber in seq(along=QA_contexts)) {
      outputIdThisQA = paste0('QA', thisQANumber)
      textareaIdThisQA = paste0('id', outputIdThisQA)
      currentValue = as.vector(input[[textareaIdThisQA]])
      cat("outputIdThisQA", outputIdThisQA, '\n==>',
          currentValue, '<==\n')
#      if( !is.na(currentValue) ) {
        cat('Filling ...\n',
            cookieValues[outputIdThisQA], '\n')
        updateTextAreaInput(
          session,
          textareaIdThisQA,
          value =
