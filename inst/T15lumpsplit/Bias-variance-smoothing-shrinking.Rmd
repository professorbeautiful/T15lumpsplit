...
output$plotLumpSplitPoints = renderPlot(
  height = 300, {
    analysisName = 'plotLumpSplitPoints'
    source('analysisReactiveSetup.R', local=TRUE)
    plotPlightPdarkPosterior(DLdata=rValues$DLdata[[analysisName]],
                           showPrior = FALSE, 
...
jumpList = c(jumpList, panelOfBifurcation = 'Testing Lump vs Split')
source('analysisInitialSetup.R', local=TRUE)
# cat('working directory (initial): ', getwd(), '\n')
# cat(
#   file.exists('analysisInitialSetup.R'), #TRUE
#   file.exists('analysisReactiveSetup.R')
#   , '\n')

output$fisherResult = renderText({
  analysisName = 'panelOfBifurcation'
  source('analysisReactiveSetup.R', local=TRUE)
  # cat('working directory (reactive): ', getwd(), '\n')
  # cat(
  #   file.exists('analysisInitialSetup.R'), #TRUE
  #   file.exists('analysisReactiveSetup.R')
  #   , '\n')
...
div(
  fluidRow(
    ...
    column(8,
           br(),
           dataTableComponent(analysisName='viewPlot')
    )
  ),
  fluidRow(
    column(4, 
...
tableOfEstimates = reactive({
  ...
  for(aN in 1:analysisNumber)
    rValues$DLdata[[analysisNumber]] = rValues$DLdataLastUsed
  nRD=rValues$DLdataLastUsed['R','D']
  nND=rValues$DLdataLastUsed['N','D']
  posterior.mean.s = (1+nRD)/(2+nRD+nND)
  nR=sum(rValues$DLdataLastUsed['R', ])
  nN=sum(rValues$DLdataLastUsed['N', ])
  posterior.mean.l = (2+nR)/(4+nN)
  
  pProbSplit = posteriorProb(theData=rValues$DLdataLastUsed)
...
observeEvent(list(input$regenerateFeatures), {
    rValues$DLdataDFwithFeatures = 
       makeDLdataWithFeatures(rValues$DLdataLastUsed)
})
```
...
```{r}
jumpList= c(jumpList, Bonferroni='Bonferroni adjustment')
dataTableComponent(showhide='hide', analysisName='Bonferroni')
...
#        rValues$qStar),  
#   {
#     rValues$BHtable = makeBHtable(rValues$allChisqPs, qStar=1)#rValues$qStar)
#   }
# )
