...
Here is a prior that represents Dr. Who's view that each one has their own person ***prior*** distribution $\pi _{Split}^{}({\bf{p}})$ or $\pi _{Lump}^{}({\bf{p}})$, and one or the other is absolutely correct, but "who knows which one?":
...
* We dont' know (and don't care about) $\phi$  
  ($\phi$, whether Dr.Lump or Dr.Split is correct, is the "nuisance parameter"), so we "marginalize" over it.  
...
Now we put a little asterisk $\pi^*$ over the $\pi$ to mean "posterior" (after seeing the data) instead of "prior" (before seeing the data).  
...
$~~~~~~~$ for Lump $\pi _{\Who}^*(\phi  = Lump)$ and Split  $\pi _{\Who}^*(\phi  = Split)$  
* the CONDITIONAL POSTERIOR probability distributions  
$~~~~~~~$ for ${\bf{p}}$ given Lump, $\pi{Lump}^*({p_{R.D}}) $ and given Split, $\pi _{Split}^*({p_{R.D}}$).  
...
### Each marginal probability of the data table, given $\phi$ (which Dr.) <!-- #title -->
